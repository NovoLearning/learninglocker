...
const streamAggregation = ({ pipeline, skip, limit, batchSize, maxTimeMS, maxScan, hint }) => {
  let query = Statement
    .aggregate(pipeline)
...
schema.statics.aggregateByAuth = function aggregateByAuth(
  ...
  return parseQuery(pipeline, {
    ...
    if (cache === false) {
      return streamAggregation({
        pipeline: finalPipeline, skip, limit, batchSize, maxTimeMS, maxScan, hint
      }).then(stream =>
